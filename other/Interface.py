...
def SettingsInit(PathToJSON):
    return Settings.Settings(PathToJSON)
...
SettingsLaunch = Settings.Settings(PathToConfig) # Initialize settings -> TODO: create a function to find this file automatically
...
if action == "create":
    ...
    if isinstance(path,pathlib.PurePath):
        ...
        LaTeXClass = Prompt.ask("Which LaTeX document class should be used ?",choices=Settings.Settings.GetClassList(SettingsLaunch),default="standard")
        ...
        if launch:
            NewProject = Project.Project(ProjectName,PathToDir,LaTeXClass)
            Project.Project.CreateProject(NewProject,images=ImagesFolder)
        else:
    ...
    if execute:
        try:
            Project.Project.RemoveProject(Project.Project(ProjectName,PathToDir,"standard"))
        except:
    ...
    if seeGlobJSON:
        config = Settings.Settings.GetSettings(SettingsLaunch)
    ...
    Sort.SortFiles(SortTarget,PathToConfig)
elif action == "save":
